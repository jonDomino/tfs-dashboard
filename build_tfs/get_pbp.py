...
def get_pbp(game_id: int) -> pd.DataFrame:
    ...
    Fetches all plays by paginating through all available pages.
    """
    # ESPN API uses pagination with default pageSize=25
    # We need to fetch all pages to get complete data
    base_url = (
        f"https://sports.core.api.espn.com/v2/sports/basketball/"
        f"leagues/mens-college-basketball/events/{game_id}/competitions/{game_id}/plays"
    )
    ...
    if page_count > 1:
        for page in range(2, page_count + 1):
            page_url = f"{base_url}?page={page}&limit=500"
            try:
                page_data = get_json(page_url)
                plays.extend(page_data.get("items", []))
            except Exception as e:
                print(f"Warning: Could not fetch page {page}: {e}")
                break
    
    print(f"Fetched {len(plays)} plays (total available: {total_count})")
    if not plays:
